...
func TestExecuteClaimedTaskRetriesAndCompletes(t *testing.T) {
	...
	if fakeStore.finalizeCalls[0].RunStatus != "failed" {
		t.Fatalf("expected first finalize to mark failed run, got %+v", fakeStore.finalizeCalls[0])
	}
	if fakeStore.finalizeCalls[1].TaskStatus != "completed" {
		t.Fatalf("expected final finalize to complete task, got %+v", fakeStore.finalizeCalls[1])
	}
	...
	if fakeJira.transitions[0] != "OPS-1->In Progress" || fakeJira.transitions[1] != "OPS-1->Done" {
